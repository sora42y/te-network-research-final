...
Table~\ref{tab:oracle} compares three specifications under GARCH+$t_5$
dynamics: raw returns (no factor removal), oracle factor-neutral (true
loadings), and estimated factor-neutral (PCA with $K=3$ factors). The
results show that factor-neutral preprocessing does not materially improve
network recovery; the $T/N$ ratio dominates estimation reliability.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccc}
    ...
    Method & $T/N=2$ & $T/N=5$ \\
    \midrule
    Raw (no factor removal) & 0.451 & 0.723 \\
    Oracle factor-neutral   & 0.253 & 0.746 \\
    Estimated (PCA)         & 0.355 & 0.667 \\
    \bottomrule
  \end{tabular}
  \medskip\\
  \small\textit{Note:} Oracle uses true factor loadings; Estimated(PCA) uses
  $K=3$ principal components under GARCH+$t_5$ idiosyncratic dynamics.
  Factor-neutral preprocessing does not materially improve network recovery
  under realistic dynamics; the $T/N$ ratio dominates. The Raw--Oracle gap
  at $T/N=5$ is modest (7.7\% vs.\ 74.6\%), and oracle performance itself
  remains below 75\% precision---confirming that Table~\ref{tab:main_results}'s
  raw-return results generalize to factor-adjusted settings. The primary
  barrier is $T/N$, not factor contamination.
\end{table}
...
Table~\ref{tab:pilot} reports quintile portfolio returns sorted on
factor-neutral OLS-TE NIO. No quintile produces significant returns;
the long-short spread reverses sign between sub-periods ($-15\%$ in
2021--2023, $+9\%$ in 2023--2026, neither significant).
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lcccccc}
    ...
    Quintile & Ann.\ Ret. & $t$
             ...
             & Ann.\ Ret. & $t$ \\
    \midrule
    Q1 (Low NIO)  & $+18.71\%$ & $1.88$ & $+11.68\%$ & $0.81$  & $+22.88\%$ & $2.11$ \\
    Q2            & $+12.08\%$ & $1.23$ & $+7.48\%$  & $0.51$  & $+16.80\%$ & $1.67$ \\
    Q3            & $+11.05\%$ & $1.14$ & $+3.42\%$  & $0.24$  & $+17.51\%$ & $1.68$ \\
    Q4            & $+7.17\%$  & $0.73$ & $+1.45\%$  & $0.10$  & $+11.18\%$ & $1.16$ \\
    Q5 (High NIO) & $+6.03\%$  & $0.59$ & $+2.70\%$  & $0.17$  & $+9.83\%$  & $1.05$ \\
    \midrule
    \textbf{L/S}
      & $\mathbf{-12.68\%}$ & $\mathbf{-2.40}$
      & $-8.98\%$ & $-1.10$
      & $-13.05\%$  & $-2.56$ \\
    \bottomrule
  \end{tabular}
  \medskip\\
  \small\textit{Note:} Long-short spread is significantly negative in both
  full sample ($t=-2.40$) and second sub-period ($t=-2.56$). High NIO stocks
  systematically underperform Low NIO stocks, suggesting estimation noise
  at $T/N \approx 0.6$ produces misleading inverse signals.
\end{table}
...
(stability selection, Benjamini-Hochberg) makes uncertainty explicit;
does not overcome $T/N$ constraints but quantifies them.
...
These results confirm that hub recovery remains poor even when edge precision
improves. LASSO-TE achieves 95\% precision at $T/N=5$ but only 30\% hub
recovery---the sparse network contains too few edges to reliably rank nodes.
