...
\medskip\noindent\textbf{JEL Classification:} G12, G14, C55, C58.\medskip\noindent\textbf{JEL Classification:} G12, G14, C55, C58.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{Power analysis: signal degradation under estimation noise.}
    We embed a known 10\% annualized network premium in simulated data and
    measure how estimation noise degrades the signal (Section~\ref{sec:oracle_nio}).
    Oracle $t=5.74$ (significant); estimated $t \approx 0.7$ at $T/N=5$ (noise).
    Even with 30\% annualized premia, signals barely emerge at $T/N=10$.
    This sets a lower bound on required $T/N$ for detectability: estimation
    noise alone destroys plausible network-return channels at typical sample sizes.
\end{enumerate}
...
Three approaches can improve reliability at low $T/N$:

\paragraph{1. Use higher-frequency data.} Hourly or 5-minute returns
