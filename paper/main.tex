...
% ============================================================
\begin{abstract}
Studies of financial networks built from transfer entropy (TE) often claim
to identify systemically important nodes and measure shock-transmission paths,
typically using samples where $T/N < 5$. We ask two questions: can these
methods recover network structure reliably, and---even if they could---does
the resulting topology predict returns?

At ratios below five, OLS-based pairwise TE recovers fewer than 17\% of true
edges and identifies hub nodes at near-random rates. LASSO achieves higher
precision but collapses to near-empty networks (density below 1\%) when applied
to S\&P 500 data. Aggregate trends are partially salvageable: crisis-period
density increases show up in every Monte Carlo trial, though attenuated. 
Node-level structure does not.

Separately, we test whether network information outflow (NIO)---a downstream
signal used in several applications---predicts cross-sectional returns. Using
the true network matrix from our simulation (oracle test), we find NIO has
zero predictive power even under perfect estimation. This points to an
identification problem distinct from the statistical one: network spillover
does not translate into tradable return predictability, regardless of how
well we measure it.

The $T/N$ barrier binds for node-level inference. The NIO mechanism may not
hold even if estimation were perfect. Both problems matter for the literature.
...
\subsection{Economic Validity: Oracle NIO Test}
\label{sec:oracle_nio}

The simulation results above establish that network topology is not
recoverable at financial $T/N$ ratios. A separate question is whether
the \emph{mechanism} holds: does network position predict returns, assuming
we could measure it perfectly?

We test this by computing NIO from the known-true adjacency matrix $A$
(no estimation error) and checking whether it predicts the forward returns
generated by the same VAR. If the NIO mechanism were valid, stocks with
high true network outflow should earn higher returns on average, since
they transmit information/shocks to others.

Table~\ref{tab:oracle_nio} shows the binary-split test results. At $T/N=5$,
the $t$-statistic is $-0.05$; at $T/N=10$, it is $-0.11$. Both are statistically
and economically zero. LASSO-based NIO performs no better ($t = -0.04$ and
$-0.03$, respectively).

\begin{table}[H]
  \centering
  \caption{Oracle NIO: Predictive Power Under Perfect Estimation}
  \label{tab:oracle_nio}
  \small
  \begin{tabular}{lcccc}
    \toprule
    & \multicolumn{2}{c}{$T/N = 5$} & \multicolumn{2}{c}{$T/N = 10$} \\
    \cmidrule(lr){2-3}\cmidrule(lr){4-5}
    Method & $t$-stat & Interpretation & $t$-stat & Interpretation \\
    \midrule
    Oracle (True $A$)  & $-0.05$ & Zero & $-0.11$ & Zero \\
    LASSO-TE           & $-0.04$ & Zero & $-0.03$ & Zero \\
    \bottomrule
  \end{tabular}
  \medskip\\
  \footnotesize
  \textit{Note:} Oracle NIO computed from true $A$ matrix; forward returns
  generated from same VAR. Negative/near-zero $t$-statistics indicate no
  predictive relationship. Signal failure persists under perfect estimation.
\end{table}

This result has two implications. First, it rules out the possibility that
better estimation methods would salvage the NIO mechanism. The problem is
not statistical noise but economic identification: in our DGP, network
information flow does not generate cross-sectional return predictability.
This could reflect market efficiency (information is priced instantly),
a mismatch between TE-based spillover and economic relevance, or the
fact that our VAR does not embed a predictable network-return channel.

Second, it clarifies what the empirical NIO failure (Section~\ref{sec:nio})
means. We cannot tell whether real-world NIO fails because (i) we measure
networks poorly, or (ii) the mechanism does not hold. Our oracle test
shows that (ii) is sufficient on its own. Prior work has attributed NIO
failure to estimation quality; we show the mechanism itself may be the issue.
...
This paper documents two independent problems in the financial network
literature: networks are not reliably estimated, and the mechanisms linking
them to outcomes may not hold.

On the statistical side, we show that at $T/N < 5$---the regime used by most
existing studies---OLS-based transfer entropy recovers fewer than 17\% of
true edges, while LASSO collapses to near-empty networks. Hub detection
performs close to random. Aggregate trends are partially salvageable, but
node-level topology is not. This is not a small-sample curiosity: even at
$T/N = 16.7$, precision remains below 20\% under realistic GARCH+factor dynamics.

On the economic side, we show that network position does not predict returns
even when measured perfectly. Our oracle NIO test---using the true adjacency
matrix from simulation---yields $t$-statistics of $-0.05$ to $-0.11$, economically
and statistically zero. This means the empirical NIO failure in Section~\ref{sec:nio}
cannot be fixed by better estimation. The mechanism itself may be absent,
whether due to market efficiency, a mismatch between TE-based information flow
and economic relevance, or structural features of financial returns that our
DGP does not capture.

These two findings are independent. The first says: we cannot measure financial
networks reliably at typical sample sizes. The second says: even if we could,
the downstream applications may not work. Prior work has conflated them, often
attributing empirical failures to ``noisy estimation'' without checking whether
the signal exists at all. We separate the issues.

For practitioners, the $T/N$ threshold is binding: node-level inference requires
$T/N \approx 8$--$10$, far above most published work. For theorists, the oracle
result suggests we need better models of how network structure (if recoverable)
connects to asset prices. Estimation quality and economic validity are distinct
constraints. Both bind.
...
\texttt{[GitHub URL]}. All simulation results are fully reproducible
